# coding=gbk
#本来想用selenium登陆京东，利用CV2识别出距离后，却死在滑块拖动上，用了两个方法都没能过京东的识别（应该不是webdriver被识别导致的，我手动试过可以验证成功）
#如果有办法我会重新上传，此代码目前无法过京东识别，参考作用
...
def move_mouse(browser,distance,element):#第一种轨迹模拟方法（三段随机速度）
    has_gone_dist=0
...
def get_tracks(browser,distance,element): #第二种轨迹模拟方法（先慢后快，最后反方向拖动，模仿人类行为）
    distance += 8
    v = 0
    t = 0.3
    forward_tracks = []
